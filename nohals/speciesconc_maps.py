...
for n in range(12):
    ...
    if variable == 'NO' or variable == 'NO2':
        ...
        im3=ax3.pcolormesh(X,Y, control-nohals,  cmap='bwr', vmin=Min, vmax=Max)

    else:
        im=ax1.pcolormesh(X,Y, control,  cmap=cmap, vmax=MAX)
        im2=ax2.pcolormesh(X,Y, nohals,  cmap=cmap, vmax=MAX)
        im3=ax3.pcolormesh(X,Y, control-nohals,  cmap='bwr', vmin=Min)
    ...
    plt.savefig('plots/SpeciesConc_map_%s_%s.png' %(variable, date)) 
    plt.close()
